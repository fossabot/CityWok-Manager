...
#             remark="REMARK",
#         )
#         response = test_client.post("/supplier/new", data=data, follow_redirects=True)
#         assert response.status_code == 200
...
#         assert len(db.session.query(Supplier).filter_by(name="FULL_INFO").all()) == 1

#     def test_basic_info(self, test_client):
#         data = dict(name="BASIC_INFO", principal="abc")
#         response = test_client.post("/supplier/new", data=data, follow_redirects=True)
...
#         assert len(db.session.query(Supplier).filter_by(name="BASIC_INFO").all()) == 1

#     def test_empty(self, test_client):
#         response = test_client.post("/supplier/new", follow_redirects=True)
#         assert response.status_code == 200
...
#         data = dict(name="BASIC_INFO", nif="123123", principal="abc")
#         response = test_client.post("/supplier/new", data=data, follow_redirects=True)
...
#         data = dict(name="BASIC_INFO", iban="PT50123123", principal="abc")
#         response = test_client.post("/supplier/new", data=data, follow_redirects=True)
...
#             "/supplier/1/update", data=data, follow_redirects=True
#         )
#         assert response.status_code == 200
#         assert request.url.endswith("/supplier/1")
#         assert db.session.query(Supplier).get(1).name == "UPDATE_INFO"
#         assert "Supplier information has been updated" in response.data.decode("utf-8")
...
#         assert os.path.isfile(f.path) == 1
